...
contract Underground is IERC20, Owned {
    ...
    {
        tFee = tAmount / 400; //0.25% transfer fee
        tTransferAmount = tAmount - tFee;
        if (_tTotal > BURN_STOP_SUPPLY) {
            tBurn = tAmount / 400; //0.25% burn fee
    ...
    /**
        Take BCH accidentally send to contract
    */
    function withdrawBCH() external onlyOwner {
        payable(owner).transfer(address(this).balance);
    }

    /**
        Take any SEP20 token sent to contract
        @param token token address
    */
    function withdrawSEP20(address token) external onlyOwner {
